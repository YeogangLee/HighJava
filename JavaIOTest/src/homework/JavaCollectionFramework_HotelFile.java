...
public class JavaCollectionFramework_HotelFile {
	private Scanner scan = new Scanner(System.in);
	private Map<Integer, Guest> hs = new HashMap<>();
	
	public static void main(String[] args) {
		new JavaCollectionFramework_HotelFile().startHotel();		
	}
		...
		while(true){			
			...
			switch(menuNum){
				...
				case 2 : 
					...
					break;
				case 3 : 
					checkHotelRoom2();		// 3.객실 상태
					break;
				case 4 : 
					saveFileHotelRoom();		// 종료
					state = false;
					displayState(state);
					return;
				default :
...
//			ex.printStackTrace();
	...
	private void saveFileHotelRoom() {
...
//					System.out.println("| 방번호: " + guest.getRoomNo() + "\t| 투숙객: " + guest.getGuestName());
				}
...
//		}else {
			...
			try {
				...
				if(hs.size() == 0 || (obj = ois.readObject()) == null) {
					System.out.println("\n현재 호텔에는 체크인 한 사람이 없습니다.");
				}else {
